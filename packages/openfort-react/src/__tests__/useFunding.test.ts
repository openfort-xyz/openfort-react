...
describe('useFunding', () => {
  ...
  describe('analytics (onEvent)', () => {
    it('emits created → payment_method_set → succeeded over a successful fund', async () => {
      const { client, create, setPaymentMethod } = makeClient()
      create.mockResolvedValue(makeSession())
      setPaymentMethod.mockResolvedValue(
        makeSession({
          ...
          paymentMethod: {
            ...
            addressUri: 'ethereum:0xrecv',
            deeplinks: [],
            cex: null,
            fees: [{ kind: 'relayerService', amount: '1', currency: '0xUSDC' }],
            minAmount: '1000',
          },
        })
      )
      ...
      const { result } = renderHook(() => useFunding({ client, onEvent }))
      await act(async () => {
        await result.current.fund(target, paymentMethod)
      })

      const types = onEvent.mock.calls.map((c) => c[0].type)
      expect(types).toEqual(['funding_session_created', 'funding_payment_method_set', 'funding_succeeded'])
      ...
      expect(onEvent).toHaveBeenCalledWith(expect.objectContaining({ type: 'funding_succeeded', txHash: '0xbeef' }))
    })

    it('emits a session_error when session creation fails', async () => {
      const { client, create } = makeClient()
      create.mockRejectedValue(new Error('boom'))
      const onEvent = vi.fn()
      ...
      expect(onEvent).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'funding_session_error', stage: 'create', sessionId: null, message: 'boom' })
      )
    })

    it('emits an abandoned event when reset() leaves a non-terminal session', async () => {
      const { client, create, setPaymentMethod } = makeClient()
      create.mockResolvedValue(makeSession())
      ...
      await act(async () => {
        void result.current.fund(target, paymentMethod).catch(() => {})
        // Flush create + setPaymentMethod so the session lands in waiting_payment,
        // but not the POLL_MS timer — no get() poll runs yet.
      ...
      expect(onEvent).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'funding_session_abandoned', lastStatus: 'waiting_payment' })
      )
    })
  })
})
