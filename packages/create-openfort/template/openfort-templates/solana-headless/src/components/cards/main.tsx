import { ClockIcon, HomeIcon, PaperAirplaneIcon, PencilSquareIcon, WalletIcon } from '@heroicons/react/24/outline'
  ...
  const tabs: TabType[] = [
    {
      ...
      component: (
        <Profile
          sampleGithubUrl="https://github.com/openfort-xyz/fortkit/tree/main/examples/quickstarts/solana-headless"
        ...
        <div className="block relative overflow-y-auto overflow-x-hidden">
          <div className="card flex-col min-h-full">
            <div className="w-full flex-1 relative">
              {tabs.map((tab) => (
                <div key={tab.name} className={`w-full flex-1 flex${currentTab.name === tab.name ? '' : ' hidden'}`}>
                  {tab.component}
                </div>
              ))}
            </div>
